...
func (b *GatewayBlockchainService) PrepareTransaction(content *ledger_model.TransactionContent) (ledger_model.PreparedTransaction, error) {
	digest, err := framework.ParseHashDigest(content.LedgerHash)
	if err != nil {
		return nil, err
	}
	contentHash, err := ledger_model.ComputeTxContentHash(crypto.GetAlgorithmByCode(b.GetCryptoSetting(digest).HashAlgorithm), content)
	if err != nil {
		return nil, err
	}
	txReqBuilder := ledger_model.NewTxRequestBuilder(contentHash, content)
	return ledger_model.NewPreparedTx(txReqBuilder, b.TxService), nil
}
